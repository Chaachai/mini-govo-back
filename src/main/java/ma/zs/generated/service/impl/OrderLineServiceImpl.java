...
public class OrderLineServiceImpl implements OrderLineService {
    ...
    @Override
    public OrderLine affectCollaborator(OrderLine orderLine) {
        if (orderLine != null && orderLine.getId() != null) {
            OrderLine dbOrderLine = findById(orderLine.getId());
            if (orderLine.getCollaborator() != null) {
                User collaborator = userService.findById(orderLine.getCollaborator().getId());
                if (collaborator != null) {
                    dbOrderLine.setCollaborator(collaborator);
                    dbOrderLine.setLongitudeCollaborator(BigDecimal.valueOf(collaborator.getLongitude()));
                    dbOrderLine.setLatitudeCollaborator(BigDecimal.valueOf(collaborator.getLatitude()));
                    OrderStatus orderStatus=orderStatusService.findByCode("AFFECTED");
                    System.out.println(orderStatus.getCode());
                    dbOrderLine.setOrderStatus(orderStatus);
                    return orderLineDao.save(dbOrderLine);
